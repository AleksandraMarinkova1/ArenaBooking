...
export default function AdminSlotsManager() {
  ...
  const handleDeleteSlot = async (id) => {
    if (!window.confirm("Дали сте сигурни дека сакате да го отстраните овој термин од системот?")) return;
  ...
  return (
    <div style={{ 
      ...
      <p style={{ margin: '0 0 20px 0', color: '#718096', fontSize: '14px' }}>
        Додавајте или отстранувајте временски опсези кои ќе им бидат достапни на корисниците за резервација.
      </p>
      
      {/* Форма за нов термин */}
      <form onSubmit={handleAddSlot} style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
        <input 
          type="text" 
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
        {slots.length === 0 ? (
          ...
          slots.map((s) => (
            ...
            >
              <span style={{ fontWeight: 'bold', color: '#4a5568', fontSize: '14px' }}>{s.slotText}</span>
              <button 
                type="button"
